"""Part I pipeline for the pendulum benchmark, as a script.
...
from mfsindy.cases.pendulum import (
    ...
    build_pendulum_weak_validation_blocks,
    fit_pendulum_multi_trajectory_rollout_models,
    generate_pendulum_dataset,
    pendulum_kappa_by_support,
    pendulum_validation_support,
    pendulum_weak_design,
...
TUNING_PATH = RESULTS_DIR / "pendulum_part1_tuning.json"
ERRORS_PATH = RESULTS_DIR / "pendulum_part1_errors.csv"
...
def make_config(**overrides) -> PendulumMultiTrajectoryGLSConfig:
    ...
    return PendulumMultiTrajectoryGLSConfig(**settings)
...
def tune(cfg, *, results_dir: Path = RESULTS_DIR, verbose: bool = True) -> TuningArtifacts:
    ...
    reference, _, _ = generate_pendulum_dataset(
    ...
    hf_tune, t_tune, _ = generate_pendulum_dataset(
    ...
    lf_tune, _, _ = generate_pendulum_dataset(
        n_traj=cfg.n_lf,
        T=cfg.T_train,
        dt=cfg.dt,
    ...
    support = pendulum_validation_support(
        cfg,
        hf_val[0],
        t_grid=t_tune,
    ...
    val_blocks = {
        seed: build_pendulum_weak_validation_blocks(
                ...
                evaluate_weak_form_models(
                    {
                        ...
                        for rung, model in fit_pendulum_multi_trajectory_rollout_models(
                            candidate,
    ...
    for rung, selection in selections.items():
        ...
        weak_design[rung] = pendulum_weak_design(rung_cfg)
...
def run_monte_carlo(cfg, selections, *, results_dir: Path = RESULTS_DIR,
    ...
    for rung, selection in selections.items():
        ...
        per_rung.results_filename = f"pendulum_part1_{rung.lower()}_errors.csv"
        run_cfgs[rung] = per_rung

    if verbose:
        print(f"running {cfg.n_runs} Monte Carlo runs per rung")
    errors = run_with_tuned_configs(run_pendulum_multi_trajectory_gls_experiment, run_cfgs)
